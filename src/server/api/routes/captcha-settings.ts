...
captchaSettingsRoutes.put("/recaptcha", zValidator("json", recaptchaSchema), async (c) => {
  const data = c.req.valid("json");

  const [existing] = await db.select().from(recaptchaSettings).limit(1);

  if (!existing) {
    const [setting] = await db.insert(recaptchaSettings).values(data).returning();
...
captchaSettingsRoutes.put("/turnstile", zValidator("json", turnstileSchema), async (c) => {
  const data = c.req.valid("json");

  const [existing] = await db.select().from(turnstileSettings).limit(1);

  if (!existing) {
    const [setting] = await db.insert(turnstileSettings).values(data).returning();
    return c.json({ data: setting }, 201);
  }
